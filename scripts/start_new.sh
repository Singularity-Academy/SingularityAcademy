...
# Start Python service (using rewrite version)
echo "Starting AI engine rewrite..."
cd "$BASE_DIR" && python3 run_rewrite.py --log-level TRACE > "$BASE_DIR/logs/python_rewrite.log" 2>&1 &
PYTHON_PID=$!
echo $PYTHON_PID >> "$PID_FILE"
echo "AI engine rewrite started with PID $PYTHON_PID"
