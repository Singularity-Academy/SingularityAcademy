"""
Authentication module for AI Engine.

This module provides authentication and authorization functionality,
including JWT token management and middleware.
"""

import os
import time
import jwt
from functools import wraps
from pathlib import Path
from sanic import json, Request, Sanic
from loguru import logger
from typing import Optional, Dict, Any

# Same default secret key as Go backend
DEFAULT_SECRET_KEY = "AISAISAAA"

class AuthError(Exception):
    """Base exception for authentication errors."""
    pass

class AuthManager:
    """
    Manages authentication for the application.
    Handles JWT token generation, validation, and user context.
    """
    
    def __init__(self, secret_key: str):
        self.secret_key = secret_key

    @classmethod
    def from_env(cls) -> "AuthManager":
        """
        Create an AuthManager instance using environment configuration.
        
        Tries to load the secret key in this order:
        1. From auth_secret.key file in the backend directory
        2. From AUTH_SECRET_KEY environment variable
        3. Falls back to DEFAULT_SECRET_KEY if neither is available
        
        Returns:
            AuthManager: Configured instance
        """
        secret_key = None
        try:
            # Try to load from key file in backend directory
            key_path = Path(__file__).parent.parent.parent.parent / "backend" / "auth_secret.key"
            if key_path.exists():
                with open(key_path, 'rb') as f:
                    secret_key = f.read().decode('utf-8').strip()
                logger.info(f"Secret key loaded from {key_path}")
        except Exception as e:
            logger.error(f"Error reading secret key file: {e}")
        
        if not secret_key:
            secret_key = os.getenv("AUTH_SECRET_KEY")
            if secret_key:
                logger.info("Secret key loaded from AUTH_SECRET_KEY environment variable.")
            
        if not secret_key:
            logger.warning("Could not load secret key from file or environment, using default key.")
            secret_key = DEFAULT_SECRET_KEY
            
        return cls(secret_key)

    def decode_token(self, token_string: str) -> dict | None:
        """
        Decodes and validates a JWT token.
        
        Args:
            token_string: The JWT token to decode
            
        Returns:
            dict | None: The token claims if valid, None otherwise
            
        Validates:
            - Token signature
            - Issuer (must be "aiLearn")
            - Expiration time
            - Presence of 'ID' claim
        """
        try:
            claims = jwt.decode(token_string, self.secret_key, algorithms=["HS256"])
            
            if claims.get("iss") != "aiLearn":
                logger.warning(f"Token validation failed: Invalid issuer '{claims.get('iss')}', expected 'aiLearn'")
                return None
                
            if "exp" in claims and claims["exp"] < int(time.time()):
                logger.warning(f"Token validation failed: Expired at {time.ctime(claims['exp'])}, current time {time.ctime()}")
                return None
                
            if "ID" not in claims:
                logger.warning("Token validation failed: Missing required 'ID' claim")
                return None
            
            return claims
        except jwt.ExpiredSignatureError:
            logger.warning("Token validation failed: Signature has expired")
            return None
        except jwt.InvalidTokenError as e:
            logger.warning(f"Token validation failed: {str(e)}")
            return None
        except Exception as e:
            logger.error(f"Unexpected error during token validation: {str(e)}")
            return None

# Create a singleton instance
auth_manager = AuthManager.from_env()

def init_auth(app: Sanic) -> None:
    """
    Initialize authentication for the Sanic application.
    
    Args:
        app: The Sanic application instance
        
    This function:
    1. Attaches the AuthManager to app.ctx.auth
    2. Registers the authentication middleware
    """
    app.ctx.auth = auth_manager
    app.middleware("request")(authentication_middleware)
    logger.info("Authentication Manager initialized and attached to app.ctx.auth")

async def authentication_middleware(request: Request) -> None:
    """
    Sanic middleware to extract and store the raw auth token.
    
    Args:
        request: The Sanic request object
        
    This middleware:
    1. Extracts the Bearer token from Authorization header
    2. Stores it in request.ctx.raw_auth_token
    3. Does not validate the token (validation happens in login_required)
    """
    auth_header = request.headers.get("Authorization")
    request.ctx.raw_auth_token = None

    if not auth_header:
        return

    parts = auth_header.split(" ")
    if len(parts) != 2 or parts[0].lower() != "bearer":
        return

    request.ctx.raw_auth_token = parts[1]

def login_required(func):
    """
    Decorator to verify authentication token and set user context.
    
    Args:
        func: The route handler to protect
        
    Returns:
        The wrapped function that checks authentication
        
    This decorator:
    1. Verifies the token is present
    2. Validates the token using AuthManager
    3. Sets user context (user_id and claims) on the request
    4. Returns 401 if authentication fails
    """
    @wraps(func)
    async def wrapper(request: Request, *args, **kwargs):
        if not request.ctx.raw_auth_token:
            return json({"error": "Authentication required", "error_code": "AUTH_REQUIRED"}, status=401)
        
        # Get auth manager from app context
        auth_manager = request.app.ctx.auth
        
        # Verify the token
        claims = auth_manager.decode_token(request.ctx.raw_auth_token)
        if not claims:
            return json({"error": "Invalid or expired token", "error_code": "INVALID_TOKEN"}, status=401)
        
        # Set user context
        request.ctx.user_id = claims.get("ID")
        request.ctx.claims = claims
        
        logger.info(f"Authorized: User {request.ctx.user_id}")
        return await func(request, *args, **kwargs)
    
    return wrapper

# Export commonly used items
__all__ = ['AuthManager', 'auth_manager', 'init_auth', 'login_required', 'AuthError']

from .models import *

